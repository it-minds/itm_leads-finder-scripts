...
def json_model_quality_control(object: JobPostingModel, id: str):
    ...
    # Check if job description is None or its length is less than 25 chars
    if not object.job_description or len(object.job_description) < 25:
        return "job description failure"
    
    # # Check if required qualifications and technologies combined are less than 3
    # if len(object.required_qualifications) + len(object.technologies) < 2:
